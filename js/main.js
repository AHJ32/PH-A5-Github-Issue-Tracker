...
function renderIssues(issues) {
  ...
  grid.querySelectorAll(".issue-card").forEach(function (card) {
    card.addEventListener("click", function () {
      var issue = allIssues.find(function (i) {
        return String(i.id) === card.dataset.id;
      });
      if (issue) {
...
searchInput.addEventListener("keydown", function (e) {
  ...
  searchIssues(q)
    ...
    .catch(function () {
      grid.innerHTML =
        '<p class="text-red-500 col-span-4 text-center py-10">Search failed.</p>';
      setCount(0);
    })
    .finally(function () {
      hideSpinner();
...
fetchAllIssues()
  ...
  .catch(function (err) {
    grid.innerHTML =
      '<p class="text-red-500 col-span-4 text-center py-10">Failed to load issues: ' +
      err.message +
      "</p>";
    setCount(0);
  })
