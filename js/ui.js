export function formatDate(dateStr) {
  ...
  function labelPills(labels) {
    ...
    return list.map((l) => {
      const cls = colorMap[l.toLowerCase()] ?? "bg-gray-100 text-gray-600 border border-gray-200";
      ...
      <div class="border-t-4 ${borderColor} -mt-1 pt-4">
        ...
        <div class="grid grid-cols-2 gap-3 text-sm">
          <div class="bg-[#f8fafc] rounded-lg p-3">
            ...
            <span class="text-[#1F2937] font-medium"><i class="ri-user-line text-[#94a3b8]"></i> ${issue.author ?? "Unknown"}</span>
          </div>
          <div class="bg-[#f8fafc] rounded-lg p-3">
          ...
          <div class="bg-[#f8fafc] rounded-lg p-3">
            <span class="text-[#94a3b8] text-xs uppercase font-semibold block mb-1">Labels</span>
            <span class="text-[#1F2937] font-medium">${(Array.isArray(issue.labels) ? issue.labels : [issue.label]).filter(Boolean).join(", ") || "N/A"}</span>
          </div>
          <div class="bg-[#f8fafc] rounded-lg p-3">
            <span class="text-[#94a3b8] text-xs uppercase font-semibold block mb-1">Created At</span>
            <span class="text-[#1F2937] font-medium"><i class="ri-calendar-line text-[#94a3b8]"></i> ${formatDate(issue.createdAt)}</span>
          </div>
        </div>
